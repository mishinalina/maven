...
#     tree. A Temurin JDK 8 is installed and becomes the default JAVA_HOME for login
...
#
# The JDK, Ant and the Maven local repository all live under /workspaces/.air-env:
# that volume is the workspace that gets snapshotted after a successful warm-up, and
# it is shared with the other containers of the environment, whereas a home directory
# is private to the user the script happens to run as.
#
# The warm-up run additionally builds the whole reactor, so the local repository and
...
WORKSPACES_DIR="$(dirname "$REPO_DIR")"
if [ -w "$WORKSPACES_DIR" ]; then
  SHARED_ROOT="$WORKSPACES_DIR/.air-env"
else
  log "WARNING: $WORKSPACES_DIR is not writable; falling back to \$HOME for the toolchains"
  SHARED_ROOT="$HOME/.air-env"
fi
JDK_HOME="$SHARED_ROOT/jdk8"
ANT_VERSION="1.10.15"
ANT_DIR="$SHARED_ROOT/ant"
M2_REPO="$SHARED_ROOT/m2-repository"
ENV_FILE="$SHARED_ROOT/shell-env.sh"
...

{
  ...
  # Guarded, so a shell never ends up pointing at a JDK that is not there.
  echo "if [ -x \"$JDK_HOME/bin/java\" ]; then"
...
hook_line="[ -f \"$ENV_FILE\" ] && . \"$ENV_FILE\"  # ${MARKER}"
...
<settings xmlns="http://maven.apache.org/SETTINGS/1.0.0">
  <!-- On the snapshotted workspace volume, so the warm-up cache survives and is
       shared by every container of the environment. -->
...
EOF
  mkdir -p "$M2_REPO"
  # A copy on the shared volume, so a shell running as another user of this
  # environment can reuse it with `mvn -s $SHARED_ROOT/settings.xml`.
  cp "$MAVEN_SETTINGS" "$SHARED_ROOT/settings.xml"
  chmod 0644 "$SHARED_ROOT/settings.xml"
  log "wrote $MAVEN_SETTINGS (local repository: $M2_REPO)"
}

write_maven_settings

# --------------------------------------------------------------------------------
# warm-up build: primes the local repository and every module's target/ directory
# for the snapshot
# --------------------------------------------------------------------------------
...
warmup_build() {
  ...
  log "building the full reactor (tests skipped) - this populates $M2_REPO and target/ ..."
